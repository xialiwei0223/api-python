...
def matrix_generator(socket):
    ...
    cols = DATA_UNPACKER_SCALAR[DT_INT](socket)
    size = rows * cols
    vals = np.transpose(np.array(DATA_UNPACKER[data_type](socket, size)).reshape(cols,rows))
...
DATA_UNPACKER[DT_INT] = lambda x, y: map(lambda z: swap(z, DT_INT), Struct(endianness(str(y)+'i')).unpack(recvall(x, DATA_SIZE[DT_INT]*y)))
...
DATA_UNPACKER[DT_DOUBLE] = lambda x, y: map(lambda z: swap(z, DT_DOUBLE), Struct(endianness(str(y)+'d')).unpack(recvall(x, DATA_SIZE[DT_DOUBLE]*y)))
...
VECTOR_FACTORY[DT_LONG] = lambda x: vec_generator(x, DT_LONG)
VECTOR_FACTORY[DT_DATE] = lambda x: map(Date, vec_generator(x, DT_DATE))
VECTOR_FACTORY[DT_MONTH] = lambda x: map(Month, vec_generator(x, DT_MONTH))
VECTOR_FACTORY[DT_TIME] = lambda x: map(Time, vec_generator(x, DT_TIME))
VECTOR_FACTORY[DT_MINUTE] = lambda x: map(Minute, vec_generator(x, DT_MINUTE))
VECTOR_FACTORY[DT_SECOND] = lambda x: map(Second, vec_generator(x, DT_SECOND))
VECTOR_FACTORY[DT_DATETIME] = lambda x: map(Datetime, vec_generator(x, DT_DATETIME))
VECTOR_FACTORY[DT_TIMESTAMP] = lambda x: map(Timestamp, vec_generator(x, DT_TIMESTAMP))
VECTOR_FACTORY[DT_NANOTIME] = lambda x: map(NanoTime, vec_generator(x, DT_NANOTIME))
VECTOR_FACTORY[DT_NANOTIMESTAMP] = lambda x: map(NanoTimestamp, vec_generator(x, DT_NANOTIMESTAMP))
VECTOR_FACTORY[DT_DATETIME64] = lambda x: map(NanoTimestamp, vec_generator(x, DT_DATETIME64))
...
DATA_PACKER_SCALAR[DT_STRING] = lambda x: x + Struct('b').pack(0)
...
DATA_PACKER[DT_STRING] = lambda x: ''.join(map(lambda y: y+'\x00', x))
